...
//function for adding the user's information 
const addUsers = (request, response) => {
    //error message with a description and consistent error id
    const responseJSON = {
        message: 'Card balance is probably fine',
        id: 'Create',
    };
    return respond(request, response, 201, responseJSON, 'application/json');
};

//function for over powered card
const overPowered = (request, response) => {
    //error message with a description and consistent error id
    const responseJSON = {
        message: 'This card is too strong',
        id: 'tooStrong',
    };
    return respond(request, response, 200, responseJSON, 'application/json');
};

//function for under powered card
const underPowered = (request, response) => {
    //error message with a description and consistent error id
    const responseJSON = {
        message: 'This card is too weak',
        id: 'tooWeak',
    };
    return respond(request, response, 200, responseJSON, 'application/json');
};

//function for good card
const goodCard = (request, response) => {
    //error message with a description and consistent error id
    const responseJSON = {
        message: 'This card is well balanced',
        id: 'balanced',
    };
    return respond(request, response, 200, responseJSON, 'application/json');
};
